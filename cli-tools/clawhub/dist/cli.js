...
import { cmdDeletePackage, cmdDeletePackageTrustedPublisher, cmdDownloadPackage, cmdExplorePackages, cmdGetPackageTrustedPublisher, cmdInspectPackage, cmdPackageModerationStatus, cmdPackageMigrationStatus, cmdPackageReadiness, cmdPackPackage, cmdPublishPackage, cmdReportPackage, cmdSetPackageTrustedPublisher, cmdTransferPackage, cmdUndeletePackage, cmdValidatePackage, cmdVerifyPackage, } from "./cli/commands/packages.js";
...
function addCliHelpHeader(command) {
...
function applyCommandHelpGroups(parent, groups) {
...
function validateTopLevelCommand(args) {
    if (hasTerminalGlobalFlag(args))
        return;
...
function hasTerminalGlobalFlag(args) {
    return args.some((arg) => arg === "--help" || arg === "-h" || arg === "--cli-version" || arg === "-V");
}
function findFirstTopLevelOperand(args) {
...
registerCommand(scanCmd, ["scan", "download"])
    ...
    const opts = await resolveGlobalOpts();
    const parentOptions = scanCmd.opts();
...
registerCommand(program, ["delete"])
    ...
    .option("--version <version>", "Permanently delete one version; cannot be restored or republished; publish a replacement first if deleting the current latest version")
...
registerCommand(program, ["sync"])
    ...
    .option("--tags <tags>", "Comma-separated tags", "latest")
    .option("--concurrency <n>", "Concurrent registry/file checks", (value) => Number.parseInt(value, 10))
    ...
    .action(async (options) => {
    const opts = await resolveGlobalOpts();
    const bump = options.bump === "patch" || options.bump === "minor" || options.bump === "major"
        ? options.bump
