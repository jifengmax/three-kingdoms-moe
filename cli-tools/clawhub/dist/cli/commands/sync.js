...
import { buildScanRoots, checkRegistrySyncState, dedupeSkillsBySlug, formatActionableLine, formatBulletList, formatCommaList, formatList, formatSyncedDisplay, formatSyncedSummary, mapWithConcurrency, normalizeConcurrency, printSection, resolvePublishMeta, scanRootsWithLabels, selectToUpload, } from "./syncHelpers.js";
export async function cmdSync(opts, options, inputAllowed) {
    ...
    if (primaryScan.skills.length === 0) {
        ...
        if (!options.dryRun && options.all) {
            fail("Refusing to publish fallback skill roots with --all. Pass --root for the skills to sync.");
        }
        if (!jsonMode) {
    ...
    try {
        ...
        const parsed = await mapWithConcurrency(skills, Math.min(concurrency, 12), async (skill) => {
            ...
            if (parsingLoader)
                parsingLoader.text = `Parsing local skills ${done}/${skills.length}`;
            return {
                ...skill,
    ...
    if (options.dryRun) {
        ...
        if (jsonMode) {
            writeSyncJson(buildSyncJsonOutput({
                ok: true,
                dryRun: true,
                registry,
                roots: outputRoots,
    ...
    if (failedUploads.length > 0) {
        ...
        printSection("Failed to publish", formatBulletList(failedUploads.map((failure) => `${failure.slug}: ${failure.message}`), 20));
        outro(`Published ${published.length} of ${selected.length} skill(s). ${failedUploads.length} failed.`);
        process.exitCode = 1;
        return;
    }
    if (jsonMode) {
    ...
    if (racedNoOps.length > 0) {
        outro(`Published ${published.length} of ${selected.length} skill(s). ${racedNoOps.length} already synced.`);
    }
    else {
...
function buildForkOf(skill, registry, ownerHandle) {
...
async function resolveSyncOwnerHandle(registry, token, owner) {
    const explicitOwner = normalizeOwner(owner);
    if (explicitOwner || !token)
        return explicitOwner;
    return await resolveDefaultOwnerHandle(registry, token);
}
function formatSyncedJson(candidate) {
    return {
        slug: candidate.slug,
...
function formatPublishJson(candidate, version, source) {
    return {
        slug: candidate.slug,
...
function buildSourceProvenance(opts, skill, options, scanRoots) {
    const rawRepo = options.sourceRepo?.trim();
    const commit = options.sourceCommit?.trim();
    if (!rawRepo && !commit && !options.sourceRef?.trim())
        return undefined;
    if (!rawRepo || !commit)
        fail("--source-repo and --source-commit must be provided together");
...
function sourcePathForSkill(opts, folder, scanRoots) {
    const bases = Array.from(new Set([opts.workdir, opts.dir, ...scanRoots, process.cwd()].map(normalizeSourcePathBase)));
    for (const base of bases) {
        const rel = relativeInside(base, folder);
        if (rel)
            return rel;
    }
    fail("Source provenance requires each skill folder to be inside the current directory, --workdir, configured skills directory, or a --root directory.");
    throw new Error("unreachable");
}
function normalizeSourcePathBase(value) {
