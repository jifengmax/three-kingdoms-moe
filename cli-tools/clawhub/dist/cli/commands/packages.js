...
const LEGACY_AUTHOR_REMEDIATION_SUMMARIES = {
    "channel-env-vars": "Move legacy channel environment variable metadata into the current setup/config metadata.",
    "legacy-before-agent-start": "Replace the legacy before_agent_start hook with the current prompt/model hooks.",
    "legacy-root-sdk-import": "Prefer focused public plugin SDK subpath imports instead of the legacy root barrel.",
    "manifest-name-missing": "Add a display name to the plugin manifest.",
    "manifest-unknown-contracts": "Remove unsupported manifest contract keys or move them to a documented OpenClaw contract field.",
    "manifest-unknown-fields": "Move unsupported top-level manifest fields into supported package metadata or remove them.",
    "package-entrypoint-missing": "Publish the entrypoint declared in OpenClaw package metadata or update the metadata to point at an existing file.",
    "package-install-metadata-incomplete": "Complete the OpenClaw install metadata so ClawHub can identify the install target.",
    "package-json-missing": "Add a package.json to the plugin package.",
    "package-manifest-version-drift": "Align the plugin version declared in package.json and openclaw.plugin.json.",
    "package-min-host-version-drift": "Set the package minimum host version to the OpenClaw version range the plugin was built and tested against.",
    "package-npm-pack-entrypoint-missing": "Include the declared OpenClaw entrypoints in the npm-packed artifact.",
    "package-npm-pack-metadata-missing": "Include OpenClaw metadata files in the npm-packed artifact.",
    "package-npm-pack-unavailable": "Make the package packable before publishing it through ClawHub.",
    "package-openclaw-entry-missing": "Declare the plugin runtime entrypoint in package.json OpenClaw metadata.",
    ...
    "package-plugin-api-compat-missing": "Declare the OpenClaw plugin API range this package supports.",
    "provider-auth-env-vars": "Move legacy provider authentication environment variables into current provider setup metadata.",
    "reserved-sdk-import": "Stop importing reserved bundled-plugin SDK compatibility paths.",
    "security-manifest-schema-unavailable": "Remove or update the unsupported security manifest schema reference.",
    "sdk-load-session-store": "Replace deprecated loadSessionStore whole-store access with row-scoped session helpers.",
    "sdk-session-file-helper": "Replace deprecated session file-path helpers with session entry and transcript identity APIs.",
    "sdk-session-store-write": "Replace deprecated whole-store session writes with row-scoped session helpers.",
    "sdk-session-transcript-file-target": "Replace legacy transcript file targets with public transcript identity or target helpers.",
    "sdk-session-transcript-low-level": "Replace low-level transcript writes with the structured transcript runtime helpers.",
    "unrecognized-security-manifest": "Remove unsupported security manifest files until OpenClaw documents a versioned security manifest schema.",
};
...
export async function cmdSetPackageTrustedPublisher(opts, packageName, options) {
    const trimmed = normalizePackageNameOrFail(packageName);
    ...
    if (!workflowFilename)
        fail("--workflow-filename required");
...
export async function cmdDeletePackageTrustedPublisher(opts, packageName, options = {}) {
    const trimmed = normalizePackageNameOrFail(packageName);
    const token = await requireAuthToken();
    const registry = await getRegistry(opts, { cache: true });
...
export async function cmdValidatePackage(opts, sourceArg, options = {}) {
    if (!sourceArg?.trim())
        fail("Path required");
    const resolvedSource = await resolveSourceInput(sourceArg, {
        workdir: opts.workdir,
    ...
    if (reportStatus(report) !== "pass") {
        ...
        throw new Error(`Plugin Inspector found ${breakageCount} hard error${breakageCount === 1 ? "" : "s"}`);
    }
}
async function createPluginInspectorConfigIfNeeded(sourcePath) {
    if ((await fileExists(join(sourcePath, "plugin-inspector.config.json"))) ||
    ...
    const rawName = packageJsonString(packageJson, "name") ??
...
function hasPackagePluginInspectorConfig(packageJson) {
    if (!packageJson)
        return false;
    return (isPlainRecord(packageJson.pluginInspector) || isPlainRecord(packageJson["plugin-inspector"]));
}
function pluginInspectorFixtureId(rawName) {
    return (rawName
        .split("/")
        .pop()
        ?.toLowerCase()
        ...
        try {
            ...
            if (plan.clawpackOnDisk) {
                if (isPackageMultipartTooLarge(payloadJson, "clawpack", [plan.clawpackOnDisk])) {
                    const staged = await uploadClawPackToStorage(registry, publishToken, plan.clawpackOnDisk, spinner);
...
function printPackageInspectorFindings(result) {
...
function renderPackageValidateTextSummary(report, paths) {
...
function collectInspectorFindings(report) {
    if (!isPlainRecord(report))
        return [];
    const keys = ["issues", "breakages", "warnings", "suggestions"];
    const findings = [];
...
function formatValidateFinding(finding) {
    const level = isInspectorBreakage(finding) ? "ERROR" : "WARNING";
    const code = typeof finding.code === "string" && finding.code.trim() ? finding.code.trim() : "unknown";
    const issueClass = typeof finding.issueClass === "string" && finding.issueClass.trim()
...
function readAuthorRemediation(finding) {
    if (!isPlainRecord(finding.authorRemediation))
        return null;
    const summary = typeof finding.authorRemediation.summary === "string"
        ? finding.authorRemediation.summary.trim()
        : "";
    const docsUrl = typeof finding.authorRemediation.docsUrl === "string"
...
function formatFindingEvidence(finding) {
    ...
    if (!Array.isArray(evidence))
        return [];
    return evidence.map(formatEvidenceValue).filter((line) => Boolean(line));
}
...
function formatReportPaths(paths) {
    const labels = [
        ["json", paths.jsonPath],
...
export async function cmdDeletePackage(opts, nameArg, options = {}, inputAllowed = true) {
    ...
    if (!options.yes) {
        ...
        const ok = await promptConfirm(version
    ...
    try {
        const result = await apiRequest(registry, {
            method: "DELETE",
            path: `${ApiRoutes.packages}/${encodeURIComponent(name)}${version ? `/versions/${encodeURIComponent(version)}` : ""}`,
            token,
...
function normalizeDeleteVersion(value) {
    if (value === undefined)
        return undefined;
...
function reportBreakageCount(report) {
    if (!isPlainRecord(report) || !isPlainRecord(report.summary))
        return 0;
    const value = report.summary.breakageCount;
    return typeof value === "number" && Number.isFinite(value) ? value : 0;
}
function filterAuthorFacingInspectorReport(report) {
    if (!isPlainRecord(report))
        return report;
    const findingKeys = ["issues", "breakages", "warnings", "suggestions"];
    if (!findingKeys.some((key) => Array.isArray(report[key])))
        return report;
    const next = { ...report };
    ...
    next.summary = {
        breakageCount,
        warningCount,
        deprecationWarningCount: authorFindings.filter((finding) => isPlainRecord(finding) && finding.issueClass === "deprecation-warning").length,
        issueCount: authorFindings.length,
    };
    return next;
}
function hasAuthorRemediation(value) {
...
function normalizeAuthorFacingInspectorFinding(value) {
    if (!isPlainRecord(value))
        return null;
    if (hasAuthorRemediation(value))
        return value;
    const code = typeof value.code === "string" ? value.code : "";
    const summary = LEGACY_AUTHOR_REMEDIATION_SUMMARIES[code];
    if (!summary)
        return null;
    return {
        ...value,
...
function isInspectorBreakage(value) {
    if (!isPlainRecord(value))
        return false;
...
function formatPackageLine(item) {
    ...
    return `${styleText(`${item.name}${version}`, "brand")}  ${item.displayName}  ${styleText(`[${flags.join(", ")}]`, "muted")}${summary}`;
}
...
function assertPackageMultipartSize(payloadJson, fileFieldName, files) {
    if (isPackageMultipartTooLarge(payloadJson, fileFieldName, files)) {
        fail(getPackageMultipartSizeError());
    }
}
function getClawPackSizeError(path) {
    return `ClawPack "${path}" exceeds 120MB limit`;
}
function isPackageMultipartTooLarge(payloadJson, fileFieldName, files) {
    return (estimatePackageMultipartUploadBytes({
        payloadJson,
        fileFieldName,
...
async function uploadClawPackToStorage(registry, publishToken, file, spinner) {
    if (spinner)
        spinner.text = `Uploading ${file.relPath}`;
    const { uploadUrl, uploadTicket } = await apiRequest(registry, {
        method: "POST",
...
function hasRealBundleManifest(fileSet) {
    return REAL_BUNDLE_MANIFESTS.some((marker) => fileSet.has(marker.path));
}
function hasLooseBundleMarker(fileSet) {
    return Array.from(fileSet).some((path) => path.startsWith("skills/") ||
...
function detectPackageFamily(fileSet, explicit) {
    if (explicit)
        return explicit;
    if (hasRealBundleManifest(fileSet))
        return "bundle-plugin";
    if (fileSet.has("openclaw.plugin.json"))
        return "code-plugin";
    if (hasLooseBundleMarker(fileSet))
        return "bundle-plugin";
    ...
    if (sourceForFetch.kind === "github") {
        ...
        if (folderStat.isFile()) {
            if (!folder.endsWith(".tgz"))
                fail("Package publish files must end in .tgz");
