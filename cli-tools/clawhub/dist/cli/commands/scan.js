...
import { ApiRoutes, ApiV1SkillScanStatusResponseSchema, ApiV1SkillScanSubmitResponseSchema, } from "../../schema/index.js";
import { requireAuthToken } from "../authToken.js";
import { getRegistry } from "../registry.js";
import { createCrabLoader, fail, formatError } from "../ui.js";
const DEFAULT_POLL_INTERVAL_MS = 2_000;
const MAX_POLL_ATTEMPTS = 900;
export async function cmdScan(opts, pathArg, options) {
    ...
    try {
        ...
        if (options.output) {
            ...
            if (!options.json)
                console.log(`Report ZIP: ${resolve(opts.workdir, options.output)}`);
        }
    }
    catch (error) {
...
export async function cmdScanDownload(opts, nameArg, options) {
    const name = nameArg.trim();
    if (!name)
        fail("Skill or plugin name required");
    const version = options.version?.trim();
    if (!version)
        fail("--version required");
    const kind = options.kind ?? "skill";
    if (kind !== "skill" && kind !== "plugin")
        fail('--kind must be "skill" or "plugin"');
    const token = await requireAuthToken();
    const registry = await getRegistry(opts, { cache: true });
    const output = resolve(opts.workdir, options.output ?? `clawhub-scan-${safeOutputName(name)}-${safeOutputName(version)}.zip`);
...
function validateScanOptions(pathArg, options) {
    const hasPath = Boolean(pathArg?.trim());
    const hasSlug = Boolean(options.slug?.trim());
    if (hasPath && hasSlug)
        fail("Choose either a local path or --slug, not both");
    if (!hasPath && !hasSlug)
        fail("Provide a local path or --slug");
    if (hasPath && options.update)
        fail("--update is only valid with --slug");
}
function rejectLocalScan() {
    fail("Local folder scans are no longer supported. Upload a new version, then run `clawhub scan download <slug> --version <version>` to retrieve stored scan results.");
}
async function submitPublishedScan(registry, token, options) {
...
async function pollScan(registry, token, scanId, spinner) {
    for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt += 1) {
        const status = await apiRequest(registry, {
            method: "GET",
...
function formatQueueProgress(status, queue) {
    ...
    if (status === "running")
        return `${queue.running}${queue.runningIsEstimate ? "+" : ""} running`;
    return undefined;
}
function printJson(status) {
    console.log(JSON.stringify(status, null, 2));
}
function printScanReport(status) {
...
function printOptional(label, value) {
    if (!value)
        return;
    console.log(`${label}: ${value}`);
}
function printFindings(value) {
    if (typeof value === "string" && value.trim()) {
        console.log(`Findings: ${value.trim()}`);
    }
}
function printAgenticRisks(value) {
