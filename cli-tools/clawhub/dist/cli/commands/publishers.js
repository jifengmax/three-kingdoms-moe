...
function normalizePublisherHandleOrFail(handle) {
    const normalized = handle.trim().replace(/^@+/, "").toLowerCase();
    if (!normalized)
        throw new Error("Publisher handle is required");
    return normalized;
}
export async function cmdCreatePublisher(opts, handle, options = {}) {
