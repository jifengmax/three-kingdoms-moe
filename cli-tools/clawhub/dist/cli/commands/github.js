...
export async function resolveSourceInput(input, options) {
...
export async function fetchGitHubSource(source) {
...
export function resolveLocalGitInfo(folder) {
    ...
    const ref = runGit(folder, ["describe", "--tags", "--exact-match"]) ||
...
async function parseGitHubUrl(input) {
    let url;
    try {
        url = new URL(input);
    }
...
function normalizeRepoSubpath(value) {
...
function resolveLocalPath(workdir, input) {
    if (input === "~")
        return homedir();
    if (input.startsWith("~/"))
...
function normalizeLocalWorkdirs(workdir, localWorkdirs) {
...
async function resolveDefaultBranch(owner, repo, token) {
    ...
    const defaultBranch = typeof parsed.default_branch === "string" ? parsed.default_branch.trim() : "";
    if (!defaultBranch)
        throw new Error("GitHub repo default branch missing");
    return defaultBranch;
}
async function resolveCommitSha(owner, repo, ref, token) {
    const response = await fetch(`${GITHUB_API}/repos/${owner}/${repo}/commits/${encodeURIComponent(ref)}`, {
        headers: buildGitHubHeaders(token),
    });
    if (!response.ok)
        throw new Error(`GitHub ref not found: ${owner}/${repo}@${ref}`);
    const parsed = (await response.json());
...
async function tryResolveCommitSha(owner, repo, ref, token) {
    const response = await fetch(`${GITHUB_API}/repos/${owner}/${repo}/commits/${encodeURIComponent(ref)}`, {
        headers: buildGitHubHeaders(token),
    });
    if (!response.ok)
...
async function downloadGitHubZip(owner, repo, ref, token) {
    const response = await fetch(`${GITHUB_API}/repos/${owner}/${repo}/zipball/${encodeURIComponent(ref)}`, {
        headers: buildGitHubHeaders(token),
    });
    if (!response.ok)
        throw new Error(`GitHub archive download failed: ${owner}/${repo}@${ref}`);
...
function stripSingleTopLevelFolder(entries) {
...
function filterEntriesForSubpath(entries, subpath) {
    if (subpath === ".")
        return entries;
...
async function resolveGitHubUrlRefAndPath(owner, repo, kind, segments) {
    if (segments.length === 0)
        throw new Error("Missing ref in GitHub URL");
    const token = process.env.GITHUB_TOKEN?.trim() || undefined;
