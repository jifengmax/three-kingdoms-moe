...
function parseSkillRef(skillArg) {
    ...
    if (slashIndex < 0)
        return { slug: normalizeSlug(value), ownerHandle: undefined };
...
function ownerScopedTransferUrl(registry, slug, ownerHandle, suffix = "") {
...
function transferRequestArgs(registry, slug, ownerHandle, suffix, token) {
    const path = `${ApiRoutes.skills}/${encodeURIComponent(slug)}/transfer${suffix}`;
    const url = ownerScopedTransferUrl(registry, slug, ownerHandle, suffix);
    return url ? { method: "POST", url, token } : { method: "POST", path, token };
}
...
async function runTransferDecision(opts, slugArg, options, inputAllowed, spec) {
    ...
    try {
        const result = await apiRequest(registry, transferRequestArgs(registry, slug, requested.ownerHandle, `/${spec.action}`, token), ApiV1TransferDecisionResponseSchema);
