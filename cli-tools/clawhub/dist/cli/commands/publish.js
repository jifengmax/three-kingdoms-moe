...
import { ApiRoutes, ApiV1PublishResponseSchema, ApiV1SkillResolveResponseSchema, ApiV1WhoamiResponseSchema, } from "../../schema/index.js";
...
export async function cmdPublish(opts, folderArg, options) {
    ...
    const tags = tagsValue
        ...
        .map((tag) => tag.trim())
        .filter(Boolean);
    const hasExplicitCatalogMetadata = options.categories !== undefined || options.topics !== undefined;
    ...
    if (!displayName)
        fail("--name required");
    if (explicitVersion && !semver.valid(explicitVersion))
    ...
    try {
        ...
        let defaultOwnerHandle;
        const getDefaultOwnerHandle = async (token) => {
            defaultOwnerHandle ??= await resolveDefaultOwnerHandle(registry, token);
            return defaultOwnerHandle;
        };
        const ownerHandle = explicitOwnerHandle ||
            (optionalToken ? await getDefaultOwnerHandle(optionalToken) : undefined);
        const sourceOwnerHandle = options.migrateOwner && ownerHandle
            ? explicitSourceOwnerHandle ||
                (optionalToken ? await getDefaultOwnerHandle(optionalToken) : undefined)
            : undefined;
        const resolved = await resolveSkillVersion(registry, slug, hashed.fingerprint, ownerHandle, optionalToken);
        ...
        const publishSourceOwnerHandle = options.migrateOwner && publishOwnerHandle
...
export async function resolveDefaultOwnerHandle(registry, token) {
    ...
    if (!handle)
        fail("Unable to resolve your publisher handle. Pass --owner explicitly.");
    return handle;
}
async function resolveSkillVersion(registry, slug, fingerprint, ownerHandle, token) {
    ...
    try {
        return await apiRequest(registry, { method: "GET", url: url.toString(), token }, ApiV1SkillResolveResponseSchema);
    }
    catch (error) {
...
function resolveAutomaticVersion(latestVersion) {
    if (!latestVersion)
        return "1.0.0";
    const nextVersion = semver.inc(latestVersion, "patch");
    if (!nextVersion)
        fail(`Latest ClawHub version is not valid semver: ${latestVersion}`);
    return nextVersion;
}
function buildPublishResult(result) {
    return { ok: true, ...result };
}
function writePublishJsonIfRequested(json, result) {
    if (json)
        process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
}
export async function prepareSkillFilesForPublish(folder) {
    return stripGeneratedSkillCards(await ensureRootManifestFile(folder, await listTextFiles(folder)));
}
...
function parseOwnerQualifiedSkillRef(value) {
    if (!value.startsWith("@"))
        return { slugAndVersion: value };
...
function splitForkSlugAndVersion(value) {
...
function buildPublishSource(options) {
    ...
    if (!rawRepo && !commit && !ref && !options.sourcePath?.trim())
        return undefined;
    if (!rawRepo || !commit)
