...
import { ApiRoutes, ApiV1SkillInstallResolveResponseSchema, ApiV1SearchResponseSchema, ApiV1SkillListResponseSchema, ApiV1SkillReportListResponseSchema, ApiV1SkillReportResponseSchema, ApiV1SkillReportTriageResponseSchema, ApiV1SkillResolveResponseSchema, ApiV1SkillResponseSchema, ApiV1SkillVersionResponseSchema, } from "../../schema/index.js";
import { extractGitHubZipPathToDir, extractZipToDir, hashSkillFiles, listManualSkills, listTextFiles, readLockfile, readSkillOrigin, writeLockfile, writeSkillOrigin, } from "../../skills.js";
...
function normalizeSkillSlugForRemote(raw) {
...
function parseSkillRefOrFail(raw) {
    const ref = raw.trim();
    if (!ref)
        fail("Slug required");
...
function isSafeSkillIdentity(value) {
    const slashIndex = value.indexOf("/");
    if (slashIndex < 0)
        return isSafeSkillSlug(value);
...
function findExistingLockKey(lock, ref) {
...
function skillRequestArgs(registry, slug, ownerHandle, token) {
    const path = `${ApiRoutes.skills}/${encodeURIComponent(slug)}`;
    const url = ownerScopedUrl(registry, path, ownerHandle);
    return url ? { method: "GET", url, token } : { method: "GET", path, token };
}
function skillVersionRequestArgs(registry, slug, version, ownerHandle, token) {
    const path = `${ApiRoutes.skills}/${encodeURIComponent(slug)}/versions/${encodeURIComponent(version)}`;
    const url = ownerScopedUrl(registry, path, ownerHandle);
    return url ? { method: "GET", url, token } : { method: "GET", path, token };
}
function withOwnerMetadata(version, installedAt, ownerHandle, existing) {
    return {
...
export async function cmdSearch(opts, query, limit) {
    ...
    try {
        ...
        for (const row of rows) {
            console.log(`${styleText(row.slug.padEnd(slugWidth), "brand")}  ${styleText(row.owner.padEnd(ownerWidth), "muted")}  ${styleText(row.name.padEnd(nameWidth), "strong")}  ${styleText(row.metric, "muted")}`);
        }
    }
    catch (error) {
...
function maxColumnWidth(values) {
...
export async function cmdInstall(opts, slug, versionFlag, force = false, forceInstall = false) {
    ...
    if (isPinnedSkillEntry(existingEntry)) {
        fail(`skill "${skillIdentity(localRef)}" is pinned; run \`clawhub unpin ${skillIdentity(localRef)}\` first`);
    }
    const spinner = createCrabLoader(`Resolving ${trimmed}`);
    try {
        // Fetch skill metadata including moderation status
        const skillMeta = await apiRequest(registry, skillRequestArgs(registry, trimmed, requested.ownerHandle, token), ApiV1SkillResponseSchema);
        const resolvedOwnerHandle = normalizeOwnerHandle(requested.ownerHandle ?? skillMeta.owner?.handle);
        ...
        let githubInstall = null;
        if (!resolvedVersion && !versionFlag) {
            const resolvedInstall = await resolveLatestSkillInstall(registry, remoteSlug, resolvedOwnerHandle, token, { forceInstall });
            if (!resolvedInstall.ok)
                fail(resolvedInstall.message);
        ...
        if (!resolvedVersion && !githubInstall)
            fail("Could not resolve latest version");
        if (versionFlag) {
            await apiRequest(registry, skillVersionRequestArgs(registry, remoteSlug, versionFlag, resolvedOwnerHandle, token), ApiV1SkillVersionResponseSchema);
        }
        if (githubInstall) {
            spinner.text = `Downloading ${trimmed} ${formatGitHubVersion(githubInstall.github.commit)}`;
            await installSkillWithOptionalStaging(target, targetExists, (installTarget) => installGitHubSkill(registry, githubInstall, installTarget));
        ...
        lock.skills[lockKey] = withOwnerMetadata(resolvedVersion, installedAt, resolvedOwnerHandle, existingEntry);
        ...
        spinner.succeed(`${styleText("Installed", "brand")} ${styleText(trimmed, "strong")} ${styleText(githubInstall ? formatGitHubVersion(resolvedVersion) : `v${resolvedVersion}`, "muted")} -> ${styleText(target, "muted")}`);
    }
    catch (error) {
        spinner.fail(formatError(error));
        throw error;
...
export async function cmdUpdate(opts, slugArg, options, inputAllowed) {
    ...
    if (!slug && !all)
        fail("Provide <skill> or --all");
    if (slug && all)
        fail("Use either <skill> or --all");
    if (options.version && !slug)
        fail("--version requires a single <skill>");
    ...
    if (requestedLockKey && isPinnedSkillEntry(lock.skills[requestedLockKey])) {
        fail(`skill "${skillIdentity(requestedRef)}" is pinned; run \`clawhub unpin ${skillIdentity(requestedRef)}\` first`);
    }
    ...
    for (const entry of slugs) {
        ...
        try {
            ...
            const requestedOwnerHandle = normalizeOwnerHandle(requestedRef?.ownerHandle ??
            ...
            const skillMeta = await apiRequest(registry, skillRequestArgs(registry, entryRef.slug, requestedOwnerHandle, token), ApiV1SkillResponseSchema);
            const resolvedOwnerHandle = normalizeOwnerHandle(requestedOwnerHandle ?? skillMeta.owner?.handle);
            const resolvedSlug = normalizeSkillSlugForRemote(skillMeta.skill?.slug) ?? entryRef.slug;
            const remoteSlug = existingOrigin?.slug ?? (requestedOwnerHandle ? entryRef.slug : resolvedSlug);
            // Check moderation status before proceeding
            if (skillMeta.moderation?.isMalwareBlocked) {
            ...
            if (!skillMeta.latestVersion && !options.version) {
                latestInstall = await resolveLatestSkillInstall(registry, remoteSlug, resolvedOwnerHandle, token, {
                ...
                if (latestInstall.installKind === "github") {
                    ...
                    const originFingerprint = existingOrigin?.slug === remoteSlug ? existingOrigin.fingerprint : undefined;
                    const hasLocalChanges = Boolean(exists &&
                        localFingerprint &&
                        (!originFingerprint || originFingerprint !== localFingerprint));
                    const matched = existingOrigin?.slug === remoteSlug &&
                        originFingerprint &&
                        localFingerprint &&
                    ...
                    if (hasLocalChanges && !options.force) {
                        ...
                        const confirm = await promptConfirm(`${entry}: local changes (no match). Overwrite with ${formatGitHubVersion(targetVersion)}?`);
                        if (!confirm) {
                            console.log(`${entry}: skipped`);
                            continue;
                        }
                    ...
                    if (matched === targetVersion && !options.force && !hasLocalChanges) {
                        if (lock.skills[entry]?.version !== targetVersion) {
                            lock.skills[entry] = withPinnedMetadata(targetVersion, lock.skills[entry]?.installedAt ?? Date.now(), lock.skills[entry]);
                            markLockDirty();
                            await flushLockfile();
                        }
                    ...
                    await installSkillWithOptionalStaging(target, exists, (installTarget) => installGitHubSkill(registry, githubResolution, installTarget));
                    const installedFiles = await listTextFiles(target);
                    const installedFingerprint = installedFiles.length > 0 ? hashSkillFiles(installedFiles).fingerprint : undefined;
                    ...
                    lock.skills[entry] = withOwnerMetadata(targetVersion, installedAt, resolvedOwnerHandle, lock.skills[entry]);
                    markLockDirty();
                    await flushLockfile();
            ...
            const latestVersion = skillMeta.latestVersion ??
                (latestInstall?.ok && latestInstall.installKind === "archive"
                    ? { version: latestInstall.archive.version }
                    : null);
            let resolveResult;
            if (localFingerprint) {
                resolveResult = await resolveSkillVersion(registry, remoteSlug, localFingerprint, resolvedOwnerHandle, token);
            }
            else {
                resolveResult = { match: null, latestVersion };
            }
            const originOwnerMatches = !resolvedOwnerHandle ||
                !existingOrigin?.ownerHandle ||
            ...
            if (matched &&
                ...
                lock.skills[entry] = withOwnerMetadata(matched, lock.skills[entry]?.installedAt ?? Date.now(), resolvedOwnerHandle, lock.skills[entry]);
                markLockDirty();
                await flushLockfile();
            }
            if (!latest) {
            ...
            if (options.version) {
                if (matched && matched === targetVersion) {
                    spinner.succeed(`${styleText(entry, "strong")} ${styleText("already at", "brand")} ${styleText(`v${matched}`, "muted")}`);
                    continue;
                }
            }
            else if (matched && semver.valid(matched) && semver.gte(matched, targetVersion)) {
                spinner.succeed(`${styleText(entry, "strong")} ${styleText("up to date", "brand")} ${styleText(`v${matched}`, "muted")}`);
                continue;
            }
            if (spinner.isSpinning) {
            ...
            markLockDirty();
            await flushLockfile();
            spinner.succeed(`${styleText("Updated", "brand")} ${styleText(entry, "strong")} -> ${styleText(`v${targetVersion}`, "muted")}`);
        }
        catch (error) {
            spinner.fail(formatError(error));
            throw error;
...
export async function cmdUnpin(opts, slug) {
    ...
    if (!isPinnedSkillEntry(existing))
        fail(`Skill "${label}" is not pinned`);
    lock.skills[lockKey] = {
...
export async function cmdUninstall(opts, slug, options = {}, inputAllowed) {
    ...
    try {
        ...
        await writeLockfile(opts.workdir, lock);
        spinner.succeed(`${styleText("Uninstalled", "brand")} ${styleText(skillIdentity(localRef), "strong")}`);
    }
    catch (error) {
        spinner.fail(formatError(error));
        throw error;
...
export function formatExploreLine(item) {
    ...
    return `${styleText(item.slug, "brand")}  ${styleText(`v${version}`, "muted")}  ${styleText(age, "muted")}${summary}`;
}
...
function resolveExploreSort(raw) {
    ...
    return fail(`Invalid sort "${raw}". Use newest, updated, rating, downloads, installs, or trending.`);
}
async function resolveSkillVersion(registry, slug, hash, ownerHandle, token) {
...
async function resolveLatestSkillInstall(registry, slug, ownerHandle, token, options = {}) {
    const path = `${ApiRoutes.skills}/${encodeURIComponent(slug)}/install`;
    const url = ownerScopedUrl(registry, path, ownerHandle);
    const requestUrl = url ? new URL(url) : null;
    if (options.forceInstall) {
        if (requestUrl)
            requestUrl.searchParams.set("forceInstall", "1");
    }
    return await apiRequest(registry, requestUrl
        ? {
            method: "GET",
...
async function installGitHubSkill(registry, resolution, target) {
...
async function installSkillWithOptionalStaging(target, targetExists, install) {
    if (!targetExists) {
...
function gitHubZipUrl(repo, commit) {
    const base = (process.env.CLAWHUB_GITHUB_CODELOAD_BASE_URL ||
        process.env.OPENCLAW_CLAWHUB_GITHUB_CODELOAD_BASE_URL ||
        "https://codeload.github.com").replace(/\/+$/, "");
...
function formatGitHubVersion(commit) {
