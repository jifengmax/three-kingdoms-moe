...
export async function cmdDeleteSkill(opts, slugArg, options, inputAllowed, labels = deleteLabels) {
    ...
    if (version && reason)
        fail("--reason/--note apply only to whole-skill deletion");
    ...
    try {
        ...
        const result = await apiRequest(registry, {
            method: "DELETE",
            path: `${ApiRoutes.skills}/${encodeURIComponent(slug)}${version ? `/versions/${encodeURIComponent(version)}` : ""}`,
            token,
            body,
...
function buildBody(reason, ownerHandle, version) {
    const body = {
        ...(version ? { version } : {}),
        ...(reason ? { reason } : {}),
...
function formatPrompt(labels, slug, version) {
    if (version) {
