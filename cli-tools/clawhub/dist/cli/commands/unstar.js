...
function unstarRequestArgs(registry, slug, ownerHandle, token) {
    const path = `${ApiRoutes.stars}/${encodeURIComponent(slug)}`;
    if (!ownerHandle)
        return { method: "DELETE", path, token };
    const url = registryUrl(path, registry);
    url.searchParams.set("ownerHandle", ownerHandle);
    return { method: "DELETE", url: url.toString(), token };
}
export async function cmdUnstarSkill(opts, slugArg, options, inputAllowed) {
    ...
    try {
        const result = await apiRequest(registry, unstarRequestArgs(registry, slug, requested.ownerHandle, token), ApiV1UnstarResponseSchema);
